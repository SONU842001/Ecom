      ...
      while($row = mysqli_fetch_assoc($purchase_product_query)){
        ...
        if($purchase_product_quantities < $product_quantity){
          echo"<script>alert('Please select less quantity.')</script>";  
            ...
            $query= " INSERT INTO carts(username,user_id,order_id,mob_no,order_product_id,order_name,order_rate,order_quantity) ";

            $query.= " VALUES('{$_SESSION['username']}','{$_SESSION['user_id']}','{$_SESSION['user_id']}','{$_SESSION['user_mobileno']}','{$purchase_product_id}','{$purchase_product_name}','{$purchase_product_sale_rate}','{$product_quantity}' ) ";
